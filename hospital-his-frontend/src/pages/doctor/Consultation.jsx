...
const API_RES_URL = (import.meta.env.VITE_API_URL || 'http://localhost:5001/api/v1') + '/';
const getConfig = () => {
...
const Consultation = () => {
    ...
    if (loading) return <div className="p-12 text-center text-text-secondary">Loading patient data...</div>;
    ...
    return (
        <div className="max-w-6xl mx-auto pb-12 relative">
            ...
            {showHistory && (
                <div className="fixed inset-0 z-50 flex justify-end">
                    ...
                    <motion.div
                        ...
                        animate={{ x: 0 }}
                        exit={{ x: '100%' }}
                        className="relative w-full max-w-md bg-surface h-full shadow-2xl p-6 overflow-y-auto"
                    >
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-bold text-text-primary">Patient History</h2>
                            <button onClick={() => setShowHistory(false)} className="p-2 hover:bg-surface-highlight rounded-full">
                                <ArrowLeft size={20} />
                            </button>
                        </div>

                        <div className="space-y-6 relative border-l-2 border-border ml-4 pl-6">
                            {patientHistory.length === 0 ? (
                                <p className="text-text-secondary italic">No previous history found.</p>
                            ) : (
                                patientHistory.map((visit) => (
                                    <div key={visit._id} className="relative mb-8">
                                        <div className="absolute -left-[33px] top-1 w-4 h-4 rounded-full bg-surface-secondary border-2 border-surface ring-2 ring-surface-secondary"></div>
                                        <div className="text-xs font-bold text-text-muted uppercase mb-1">{new Date(visit.scheduledDate).toLocaleDateString()}</div>
                                        <div className="bg-surface-secondary p-4 rounded-xl border border-border">
                                            <div className="font-bold text-text-primary mb-2">{visit.type || 'OPD Visit'}</div>
                                            {visit.diagnosis && (
                                                <div className="mb-2">
                                                    <div className="text-xs text-text-muted uppercase">Diagnosis</div>
                                                    <div className="text-text-primary">{visit.diagnosis}</div>
                                                </div>
                                            )}
                                            {visit.chiefComplaint && (
                                                <div className="mb-2">
                                                    <div className="text-xs text-text-muted uppercase">Symptoms</div>
                                                    <div className="text-text-secondary text-sm">{visit.chiefComplaint}</div>
                                                </div>
                                            )}
                                            {visit.prescription && visit.prescription.length > 0 && (
                                                <div className="mt-3 pt-3 border-t border-border">
                                                    ...
                                                    <ul className="text-sm text-text-secondary list-disc ml-4">
            ...
            <div className="flex items-center gap-4 mb-6">
                <button onClick={() => navigate(-1)} className="p-2 hover:bg-surface-highlight rounded-full text-text-secondary">
                    <ArrowLeft size={20} />
                </button>
                <div>
                    <h1 className="text-2xl font-bold text-text-primary">Consultation</h1>
                    <p className="text-text-secondary text-sm">Token #{appointment.tokenNumber || 'N/A'} • {new Date().toLocaleDateString()}</p>
                </div>
                <div className="ml-auto flex gap-3">
                    <button onClick={fetchHistory} className="px-4 py-2 border border-border rounded-lg text-text-secondary hover:bg-surface-highlight">View History</button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-surface p-6 rounded-2xl border border-border shadow-sm">
                        <div className="flex items-center gap-4 mb-4">
                            <div className="w-16 h-16 rounded-full bg-blue-500/10 text-blue-500 flex items-center justify-center text-2xl font-bold">
                                {appointment.patient.firstName[0]}{appointment.patient.lastName[0]}
                            </div>
                            <div>
                                <h2 className="text-lg font-bold text-text-primary">{appointment.patient.firstName} {appointment.patient.lastName}</h2>
                                <p className="text-sm text-text-secondary">{appointment.patient.patientId}</p>
                            </div>
                        </div>
                        <div className="space-y-3 text-sm text-text-secondary">
                            <div className="flex justify-between border-b border-border pb-2">
                                <span>Age / Gender</span>
                                <span className="font-medium text-text-primary">{appointment.patient.age || 'N/A'} / <span className="capitalize">{appointment.patient.gender}</span></span>
                            </div>
                            <div className="flex justify-between border-b border-border pb-2">
                            ...
                            <div className="flex justify-between">
                                <span>Contact</span>
                                <span className="font-medium text-text-primary">{appointment.patient.phone}</span>
                            </div>
                        </div>
                    </div>
                            ...
                            <>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="p-3 bg-surface-secondary rounded-lg">
                                        <div className="text-xs text-text-muted uppercase">BP</div>
                                        <div className="font-bold text-text-primary">
                                            {vitals.bloodPressure?.systolic || '--'}/{vitals.bloodPressure?.diastolic || '--'}
                                        </div>
                                    </div>
                                    <div className="p-3 bg-surface-secondary rounded-lg">
                                        <div className="text-xs text-text-muted uppercase">Heart Rate</div>
                                        <div className="font-bold text-text-primary">{vitals.pulse?.rate || '--'} bpm</div>
                                    </div>
                                    <div className="p-3 bg-surface-secondary rounded-lg">
                                        <div className="text-xs text-text-muted uppercase">Temp</div>
                                        <div className="font-bold text-text-primary">{vitals.temperature?.value || '--'}°C</div>
                                    </div>
                                    <div className="p-3 bg-surface-secondary rounded-lg">
                                        <div className="text-xs text-text-muted uppercase">SpO₂</div>
                                        <div className="font-bold text-text-primary">
                                    ...
                                    <div className="p-3 bg-surface-secondary rounded-lg">
                                        <div className="text-xs text-text-muted uppercase">Resp Rate</div>
                                        <div className="font-bold text-text-primary">{vitals.respiratoryRate?.rate || '--'}/min</div>
                                    </div>
                                    <div className="p-3 bg-surface-secondary rounded-lg">
                                        <div className="text-xs text-text-muted uppercase">Consciousness</div>
                                        <div className="font-bold text-text-primary capitalize">{vitals.avpuScore?.replace('_', ' ') || 'Alert'}</div>
                                    </div>
                                </div>
                            </>
                        ) : (
                ...
                <div className="lg:col-span-2 space-y-6">
                                ...
                                {inputMode === 'type' ? (
                                    ...
                                    >
                                        <div className="relative">
                                            ...
                                            <textarea
                                                className="w-full bg-transparent border-none focus:ring-0 p-0 text-text-primary text-lg leading-8 resize-none font-medium placeholder-teal-800/20"
                                        ...
                                        <div className="relative">
                                            ...
                                            <input
                                                className="w-full bg-transparent border-none focus:ring-0 p-0 text-text-primary text-xl leading-8 font-bold placeholder-teal-800/20"
                                                style={{ lineHeight: '32px' }}
                                                value={diagnosis}
                                                onChange={(e) => {
                    ...
                    >
                        ...
                        {/* Rx Input */}
                        <div className="grid grid-cols-12 gap-2 mb-6 items-center bg-surface p-2 rounded-xl shadow-xs border border-emerald-500/20 relative z-10">
                            ...
                            <div className="col-span-2 border-l border-border">
                                <input
                                    placeholder="Dose"
                                    className="w-full bg-transparent border-none focus:ring-0 px-3 py-2 text-text-primary placeholder-text-muted"
                                    value={newMed.dosage}
                            ...
                            <div className="col-span-2 border-l border-border">
                                <select
                                    className="w-full bg-transparent border-none focus:ring-0 px-2 py-2 text-text-primary text-sm"
                            ...
                            <div className="col-span-2 border-l border-border">
                                <input
                                    ...
                                    className="w-full bg-transparent border-none focus:ring-0 px-2 py-2 text-text-primary placeholder-text-muted text-sm"
                        ...
                        <div className="space-y-3 relative z-10">
                            {medicines.map((med, idx) => (
                                <motion.div
                                    ...
                                    animate={{ opacity: 1, x: 0 }}
                                    className="flex justify-between items-center p-4 bg-surface rounded-xl border border-emerald-500/20 shadow-sm"
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="font-bold text-text-primary text-lg leading-tight">{med.name}</div>
                                            <div className="text-xs text-text-muted font-medium mt-0.5">{med.dosage}</div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-6">
                                        <div className="flex flex-col items-end">
                                            <span className="text-sm font-bold text-emerald-500 bg-emerald-500/10 px-2 py-0.5 rounded-md">{med.frequency}</span>
                                            <span className="text-xs text-text-muted mt-1">{med.duration}</span>
                                        </div>
                                        <button onClick={() => removeMedicine(med.id)} className="p-2 text-text-muted hover:text-red-500 transition-colors">
                                            <Trash size={18} />
                                        </button>
                                    </div>
                                </motion.div>
                    ...
                    >
                        ...
                        <div className="flex gap-3 mb-6 relative z-10">
                            <select
                                className="flex-1 px-4 py-2.5 bg-surface border border-blue-500/20 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none text-text-primary"
                        ...
                        <div className="space-y-3 relative z-10">
                            {labTests.map((test, idx) => (
                                <motion.div
                                    ...
                                    initial={{ opacity: 0, x: -10 }}
                                    animate={{ opacity: 1, x: 0 }}
                                    className="flex justify-between items-center p-4 bg-surface rounded-xl border border-blue-500/20 shadow-sm"
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="font-bold text-text-primary">{test.testName}</div>
                                            <div className="text-xs text-text-muted font-mono">{test.testCode}</div>
                                        </div>
                                    </div>
                                    <button onClick={() => removeTest(test._id)} className="p-2 text-text-muted hover:text-red-500 transition-colors">
                                        <Trash size={18} />
                                    </button>
                                </motion.div>
                            ))}
                    ...
                    >
                        ...
                        <div className="flex gap-3 mb-6 relative z-10">
                            <select
                                className="flex-1 px-4 py-2.5 bg-surface border border-violet-500/20 rounded-xl focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 outline-none text-text-primary"
                        ...
                        <div className="space-y-3 relative z-10">
                            {radiologyTests.map((test, idx) => (
                                <motion.div
                                    ...
                                    initial={{ opacity: 0, x: -10 }}
                                    animate={{ opacity: 1, x: 0 }}
                                    className="flex justify-between items-center p-4 bg-surface rounded-xl border border-violet-500/20 shadow-sm"
                                >
                                    <div className="flex items-center gap-4">
                                        <div className="w-6 h-6 rounded-full bg-violet-500/10 text-violet-500 flex items-center justify-center text-xs font-bold">
                                            {idx + 1}
                                        </div>
                                        <div>
                                            <div className="font-bold text-text-primary">{test.testName}</div>
                                            <div className="text-xs text-text-muted">
                                                ...
                                                {test.modality && <span className="ml-2 px-1.5 py-0.5 bg-violet-500/10 text-violet-500 rounded text-[10px] font-medium">{test.modality}</span>}
                                            </div>
                                        </div>
                                    </div>
                                    <button onClick={() => removeRadiologyTest(test._id)} className="p-2 text-text-muted hover:text-red-500 transition-colors">
                                        <Trash size={18} />
                                    </button>
                                </motion.div>
                            ))}
                    ...
                    {!activeAdmission && (
                        <motion.div
                            ...
                            className="bg-surface-secondary p-6 rounded-2xl shadow-sm border border-border relative overflow-hidden"
                        >
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h3 className="font-semibold text-text-secondary">Care Plans</h3>
