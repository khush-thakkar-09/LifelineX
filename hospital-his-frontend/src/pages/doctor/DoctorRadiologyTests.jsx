    ...
    const getScanImageUrl = () => {
        if (selectedTest?.scanImage) {
            const imagePath = selectedTest.scanImage.startsWith('/')
                ...
                : '/' + selectedTest.scanImage;
            return `${(import.meta.env.VITE_API_URL ? import.meta.env.VITE_API_URL.replace('/api/v1', '') : (import.meta.env.VITE_API_URL ? import.meta.env.VITE_API_URL.replace('/api/v1', '') : 'http://localhost:5001') + '')}${imagePath}`;
        }
        return null;
    };
    ...
    const getReportUrl = () => {
        if (selectedTest?.reportUrl) {
            const reportPath = selectedTest.reportUrl.startsWith('/')
                ...
                : '/' + selectedTest.reportUrl;
            return `${(import.meta.env.VITE_API_URL ? import.meta.env.VITE_API_URL.replace('/api/v1', '') : (import.meta.env.VITE_API_URL ? import.meta.env.VITE_API_URL.replace('/api/v1', '') : 'http://localhost:5001') + '')}${reportPath}`;
        }
        return null;
    };
