...
const API_URL = (import.meta.env.VITE_API_URL || 'http://localhost:5001/api/v1') + '/patients/';
...
const getPatientLabResults = async (patientId) => {
    const response = await axios.get(`${import.meta.env.VITE_API_URL || (import.meta.env.VITE_API_URL || 'http://localhost:5001/api/v1') + ''}/lab/orders?patient=${patientId}`, getConfig());
    return response.data.data;
};
